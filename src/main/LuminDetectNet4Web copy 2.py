...
if process_button and st.session_state['uploaded_files_data']:
    ...
    if processed_images.size > 0: # Check if any images were processed
        ...
        with st.spinner("Classifying images as clear or dark shots..."):
            ...
            # Get the indexes where the value is 1
            ind_clear_shots = np.where(log_predictions == 1) # indexes of the images classified as clear shots
            st.success("Logistic classification completed!")

        clear_shot_images = processed_images[ind_clear_shots] # The CNN is supposed to process clear shots only
        if clear_shot_images.size > 0: # Check if any images were processed
        ...
        contImgCNN = 0 # auxiliar counter used to index the results of the CNN
        for contImg in range(len(file_names)): # iterates over the dataset
            ...
            if log_pred_value==1: # In case the clear is show, the file was processed by the CNN
                # Get CNN result
                cnn_pred_prob_array = cnn_predictions[contImgCNN]
                cnn_pred_prob_val   = cnn_pred_prob_array.item() # item() converts a single-element NumPy array into a scalar
            ...
            st.session_state['processing_results'].append({
                ...
                "cnn_pred_class": cnn_pred_class, # Class of the Conv Net model (with/wihout waves patterns)
                "cnn_pred_value": cnn_pred_value,  # Output value of the Conv Net model (rounded)
                "image_data": original_images[contImg] # Store the original image here                
