...
if process_button and st.session_state['uploaded_files_data']:
    ...
    if processed_images.size > 0: # Check if any images were processed
        ...
        with st.spinner("Classifying images as clear or dark shots..."):
            ...
            ind_clear_shots = np.where(log_predictions == 1)[0] # Stores the indices of all "clear shots"
            st.success("Logistic classification completed!") 

        clear_shot_images = processed_images[ind_clear_shots] # Stores the clear shot images
        ...
        if clear_shot_images.size > 0:
            ...
            with st.spinner("Classifying images for waves patterns..."):
                cnn_predictions = CONV_NET.predict(clear_shot_images) # Processes the "clear shots"
        ...
        for contImg in range(len(file_names)): # iterates over all uploaded images
            ...
            log_pred_value = log_predictions[contImg] # Classification of the image, 1 for "clear shots", 0 otherwise
            ...
            if log_pred_value == 1: # if "file_name" contains a clear shot
                if contImgCNN < len(cnn_predictions):
                    ...
                    contImgCNN += 1 # Since contImgCNN refers to a clear shot, computes next CNN prediction's index
                else:
...
if st.session_state['processing_results']:
    ...
    if st.button("Generate ZIP with All Classified Images"):
    ...
    # Button to trigger ZIP creation for the images with the waves ONLY
    # This button prepares the zip file and stores it in session state
    if st.button("Generate ZIP with Wave Images ONLY"):
        with st.spinner("Creating zip archive... This may take a moment for large datasets."):
            st.session_state['waves_zip_buffer'] = create_images_with_waves_zip()
        if st.session_state['waves_zip_buffer']:
            st.success("Zip archive created! Click the button below to download.")
        else:
            st.error("Failed to create zip archive.")
    ...
    if st.session_state['classified_zip_buffer']:
        st.download_button(
            label="Download ALL Classified Images (ZIP)",
    ...
    # Download button for the WAVES-ONLY ZIP
    if st.session_state['waves_zip_buffer']:
        st.download_button(
            label="Download the Images with Waves Detected (ZIP)",
